...
import { BookOpen, FileUp, LockKeyhole, Plus, Save, ShieldCheck, Upload } from "lucide-react";
...
type DisciplinaDisponivel = { id: string; nome: string; slug: string; marca: string };
...
export function CursoPrivadoClient({ disciplinasDisponiveis }: { disciplinasDisponiveis: DisciplinaDisponivel[] }) {
  ...
  const porId = useMemo(() => new Map(disciplinasDisponiveis.map((disciplina) => [disciplina.id, disciplina])), [disciplinasDisponiveis]);
  const aliasesDisciplinas = useMemo(() => new Map(disciplinasDisponiveis.flatMap((disciplina) => [
    [normalizarDisciplina(disciplina.id), disciplina.id] as const,
    [normalizarDisciplina(disciplina.nome), disciplina.id] as const,
    [normalizarDisciplina(disciplina.slug), disciplina.id] as const,
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 sm:py-10">
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(340px,0.85fr)]">
        <form onSubmit={salvarManual} className="rounded-2xl border border-border bg-surface p-5 sm:p-6" style={{ boxShadow: "var(--shadow)" }}>
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            <Campo label="Disciplina"><select required value={rascunho.disciplinaId} onChange={(e) => alterar("disciplinaId", e.target.value)} className={input}><option value="">Selecione</option>{disciplinasDisponiveis.map((disciplina) => <option key={disciplina.id} value={disciplina.id}>{disciplina.nome}</option>)}</select></Campo>
      ...
      <section className="mt-6 rounded-2xl border border-border bg-surface p-5 sm:p-6" style={{ boxShadow: "var(--shadow)" }}>
        <h2 className="flex items-center gap-2 text-lg font-bold text-text"><BookOpen className="size-5 text-accent" /> Disciplinas registradas</h2>
        {!disciplinas.length ? <p className="mt-3 text-sm text-text-muted">Nenhuma disciplina privada registrada ainda.</p> : <div className="mt-4 grid gap-3 md:grid-cols-2">{disciplinas.map((item) => { const disciplina = porId.get(item.disciplinaId); return <article key={item.disciplinaId} className="rounded-xl border border-border bg-surface-2 p-4"><div className="flex items-start gap-3"><span className="grid size-8 place-items-center rounded-lg bg-accent-soft text-xs font-bold text-accent">{disciplina?.marca ?? "CM"}</span><div className="min-w-0 flex-1"><h3 className="font-semibold text-text">{disciplina?.nome ?? item.disciplinaId}</h3><p className="mt-1 text-xs text-text-muted">{item.periodo ? `${item.periodo}o periodo` : "Periodo nao informado"} - {STATUS_LABEL[item.status]}{item.dificuldade ? ` - dificuldade ${item.dificuldade}/5` : ""}</p>{item.observacao && <p className="mt-2 line-clamp-2 text-xs leading-5 text-text-muted">{item.observacao}</p>}</div></div><div className="mt-3 flex gap-3 text-xs font-semibold"><button onClick={() => editar(item)} className="text-accent hover:underline">Editar</button>{disciplina && <Link href={`/biblioteca/${disciplina.slug}`} className="text-text-muted hover:text-accent hover:underline">Ver conteudo</Link>}</div></article>; })}</div>}
      </section>
