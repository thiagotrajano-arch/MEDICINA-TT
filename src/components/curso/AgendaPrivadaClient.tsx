...
import { CalendarDays, Check, ChevronLeft, ChevronRight, Clock3, LockKeyhole, Plus, RefreshCw, Search, Trash2 } from "lucide-react";
...
export function AgendaPrivadaClient() {
  ...
  const carregar = async (referencia = mes) => {
    ...
    catch (erro) { setMensagem(erro instanceof Error ? erro.message : "Não foi possível carregar sua agenda."); }
  ...
  const porDia = useMemo(() => {
    ...
    for (const item of itens) { const dia = chaveDia(new Date(item.inicio)); mapa.set(dia, [...(mapa.get(dia) ?? []), item]); }
    return mapa;
  }, [itens]);
  const dias = useMemo(() => {
