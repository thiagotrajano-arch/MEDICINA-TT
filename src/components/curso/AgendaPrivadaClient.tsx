...
export function AgendaPrivadaClient({ disciplinas }: { disciplinas: Disciplina[] }) {
  ...
  if (autenticado === null) return <div className="legacy-page max-w-5xl"><p role="status" aria-live="polite" className="legacy-loading">Verificando sua sessão...</p></div>;
  if (!autenticado) return <div className="legacy-page max-w-3xl"><section className="legacy-hero p-6"><LockKeyhole className="size-7 text-accent" /><h1 className="mt-3 text-2xl font-bold text-text">Agenda privada</h1><p className="mt-2 text-sm text-text-muted">Entre pela conta para ver ou registrar sua rotina. Nenhum evento é solicitado sem sessão.</p><Link href="/" className="mt-4 inline-flex text-sm font-semibold text-accent">Voltar ao início</Link></section></div>;
