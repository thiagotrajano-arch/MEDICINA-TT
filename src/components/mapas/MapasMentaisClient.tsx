...
export function MapasMentaisClient({ mapas }: { mapas: Mapa[] }) {
  ...
  return <>
    <div className="mt-6 grid gap-3 rounded-2xl border border-border bg-surface p-4 sm:grid-cols-[minmax(0,1fr)_auto_auto]"><label className="flex h-10 items-center gap-2 rounded-lg border border-border bg-bg px-3"><Search className="size-4 text-text-faint" /><input value={busca} onChange={(e) => { setBusca(e.target.value); setLimite(8); }} placeholder="Buscar diagnóstico, conceito ou ramo" className="w-full bg-transparent text-sm text-text outline-none placeholder:text-text-faint" /></label><select value={disciplina} onChange={(e) => { setDisciplina(e.target.value); setLimite(8); }} className="h-10 rounded-lg border border-border bg-bg px-3 text-sm text-text"><option value="todas">Todas as disciplinas</option>{disciplinas.map(([id, nome]) => <option key={id} value={id}>{nome}</option>)}</select><button onClick={() => { setAlto((v) => !v); setLimite(8); }} className={`h-10 rounded-lg border px-3 text-sm font-semibold ${alto ? "border-accent bg-accent-soft text-accent" : "border-border bg-bg text-text-muted"}`}><Sparkles className="mr-1 inline size-3.5" />Alto rendimento</button></div>
    <div className="mt-4 flex items-center justify-between gap-3 text-xs text-text-faint"><span>{filtrados.length} mapas encontrados · mostrando {visiveis.length}</span><span className="hidden sm:inline">Cada mapa: núcleo → ramos → estudo completo</span></div>
...
function MapaVisual({ mapa }: { mapa: Mapa }) {
  ...
  const ramo = (texto: string, indice: number, lado: "esquerda" | "direita") => <Link href={url} key={`${lado}-${texto}`} className={`mindmap-node ${CORES[indice % CORES.length]} ${lado === "esquerda" ? "mindmap-node-left" : "mindmap-node-right"}`}><span className="mindmap-arrow" aria-hidden="true">{lado === "esquerda" ? "→" : "←"}</span><span>{texto}</span></Link>;
