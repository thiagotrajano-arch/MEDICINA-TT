...
export function MidiaPrivadaClient() {
  ...
  const indiceSubtemas = useMemo(() => {
    const grupos = new Map<string, { disciplina: string; tema: string; subtema: string; quantidade: number; resumo: string }>();
    for (const item of itens) {
      if (!item.subtema) continue;
      if (filtroDisciplina !== "todas" && item.disciplina !== filtroDisciplina) continue;
      if (filtroTema !== "todos" && item.tema !== filtroTema) continue;
      const chave = `${item.disciplina}\u0000${item.tema}\u0000${item.subtema}`;
      ...
      if (atual) atual.quantidade += 1;
        ...
        quantidade: 1,
        resumo: item.observacao || item.diagnostico || "Abra este subtema para estudar as imagens com contexto e procedência.",
      });
    }
  ...
  return <div className="legacy-page max-w-6xl">
    ...
    <section className="mt-6 rounded-2xl border border-border bg-surface p-5 sm:p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="flex items-center gap-2 text-lg font-bold text-text"><FileImage className="size-5 text-accent" /> Acervo visual</h2><p className="text-sm text-text-muted">{filtrados.length} de {itens.length} imagem(ns) visível(is) nesta conta.</p></div><button disabled={ocupado} onClick={() => void carregar()} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm text-text-muted"><RefreshCw className="size-4" />Atualizar</button></div>
      ...
      {!itens.length ? <p className="mt-5 rounded-xl bg-surface-2 p-4 text-sm text-text-muted">Nenhuma imagem cadastrada ainda.</p> : filtroSubtema === "todos" ? <div className="mt-5"><p className="rounded-xl border border-border bg-surface-2 p-4 text-sm text-text-muted">Escolha um subtema. As imagens só serão reveladas dentro dele, acompanhadas de contexto e procedência.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{indiceSubtemas.map((grupo) => <button key={`${grupo.disciplina}:${grupo.tema}:${grupo.subtema}`} type="button" onClick={() => { setFiltroDisciplina(grupo.disciplina); setFiltroTema(grupo.tema); setFiltroSubtema(grupo.subtema); }} className="rounded-xl border border-border bg-surface-2 p-4 text-left transition-colors hover:border-accent"><p className="text-[10px] font-semibold uppercase tracking-wide text-accent">{grupo.disciplina} · {grupo.tema}</p><h3 className="mt-1 font-bold text-text">{grupo.subtema}</h3><p className="mt-2 line-clamp-2 text-xs leading-5 text-text-muted">{grupo.resumo}</p><p className="mt-3 text-xs font-semibold text-accent">Abrir {grupo.quantidade} {grupo.quantidade === 1 ? "imagem" : "imagens"}</p></button>)}</div></div> : !filtrados.length ? <p className="mt-5 rounded-xl bg-surface-2 p-4 text-sm text-text-muted">Nenhuma imagem corresponde aos filtros atuais.</p> : <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{filtrados.map((item) => { const resumo = hrefResumo(item); return <article key={item.id} className="overflow-hidden rounded-xl border border-border bg-surface-2">{item.urlTemporaria ? <button type="button" onClick={() => setSelecionado(item)} className="group relative block w-full cursor-zoom-in bg-bg" aria-label={`Ampliar ${item.titulo}`}><NextImage src={item.urlTemporaria} alt={item.titulo} width={1200} height={900} unoptimized className="aspect-[4/3] w-full object-contain transition-transform group-hover:scale-[1.02]" /><span className="absolute bottom-2 right-2 inline-flex items-center gap-1 rounded-full bg-black/70 px-2.5 py-1 text-[11px] font-semibold text-white"><Maximize2 className="size-3" />Ampliar</span></button> : <div className="grid aspect-[4/3] place-items-center bg-bg text-sm text-text-muted"><ImageIcon className="size-8" />Visualização expirada</div>}<div className="p-4"><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-semibold uppercase text-accent">{ROTULOS[item.tipoOrigem]}</p><span className="rounded-full bg-surface px-2 py-0.5 text-[10px] font-semibold text-text-faint">{STATUS_TRIAGEM[item.triagemStatus]}</span></div><h3 className="mt-1 font-bold text-text">{item.titulo}</h3><p className="mt-2 text-xs text-text-muted">{[item.disciplina, item.tema, item.subtema, item.modalidade].filter(Boolean).join(" · ") || "Sem classificação"}</p>{(item.periodo || item.caso) && <p className="mt-2 text-xs text-text-muted">{item.periodo ? `${item.periodo}º período` : ""}{item.periodo && item.caso ? " · " : ""}{item.caso ? `Caso: ${item.caso}` : ""}</p>}{item.diagnostico && <p className="mt-2 text-xs text-text-muted">Achado: {item.diagnostico}</p>}{item.fonte && <p className="mt-2 text-xs text-text-muted">Origem: {item.fonte}{item.pagina ? `, p. ${item.pagina}` : ""}</p>}{item.observacao && <p className="mt-3 text-sm leading-6 text-text-muted"><strong className="text-text">Como interpretar:</strong> {item.observacao}</p>}<div className="mt-3 flex flex-wrap items-center gap-3 text-xs font-semibold">{resumo && <Link href={resumo} className="inline-flex items-center gap-1 text-accent hover:underline"><BookOpen className="size-3" />Abrir resumo</Link>}<button disabled={ocupado} onClick={() => void excluir(item)} className="inline-flex items-center gap-1 text-red-600"><Trash2 className="size-3" />Remover</button></div></div></article>; })}</div>}
    </section>
